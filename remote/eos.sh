...
REPO_URL=" -u https://gitlab.e.foundation/e/os/android.git -b v4.0-a16 --git-lfs --depth=1 --no-tags --no-clone-bundle "
...
git clone https://gitlab.e.foundation/steadfasterX/android_vendor_e vendor/e
...
echo 'export EOS_DEVICE=chime' >> vendorsetup.sh
echo 'export EOS_USE_CCACHE=1' >> vendorsetup.sh
echo 'export EOS_CCACHE_COMPRESS=true' >> vendorsetup.sh
echo 'export EOS_CCACHE_DIR=/home/user/.ccache' >> vendorsetup.sh
echo 'export EOS_CCACHE_SIZE=80G' >> vendorsetup.sh
echo 'export EOS_SIGNATURE_SPOOFING=restricted' >> vendorsetup.sh
echo 'export EOS_RELEASE_TYPE=UNOFFICIAL' >> vendorsetup.sh
chmod +x vendorsetup.sh
echo '$(call inherit-product, vendor/e/config/common.mk)' >> lineage_chime.mk

git revert --no-edit ea4aba08985fe0addebcaed19a86e86bad64239c # Squiggly media progress bar, depends on ROM
#git revert --no-edit 635ae228dcb4dda3545d0778d710ecadd8a643da # vibrator
...
mka installclean
mka eos -j $(nproc --all)       ; check_fail
