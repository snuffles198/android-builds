...
DEVICE_TREE_CMD="git clone https://github.com/Joe7500/device_xiaomi_chime.git -b $DEVICE_BRANCH device/xiaomi/chime"
VENDOR_TREE_CMD="git clone https://github.com/Joe7500/vendor_xiaomi_chime.git -b $VENDOR_BRANCH vendor/xiaomi/chime"
KERNEL_TREE_CMD="git clone https://github.com/Joe7500/kernel_xiaomi_chime_prebuilt kernel/xiaomi/chime"
TOOLCHAIN_CMD='curl -o toolchain.tar.xz -L "https://github.com/Joe7500/Builds/releases/download/Stuff/toolchain.tar.xz"'
...
BUILD_LOCATION=crave.io
if [ "$BUILD_LOCATION" == "local" ] ; then
   source ~/android-builds/etc/local-conf.sh
fi
...
notify_send "Build $PACKAGE_NAME on $BUILD_LOCATION started."
...
check_fail () {
   if [ $? -ne 0 ]; then 
       if ls out/target/product/chime/$PACKAGE_NAME*.zip; then
   	  notify_send "Build $PACKAGE_NAME on $BUILD_LOCATION softfailed."
          echo weird. build failed but OTA package exists.
          echo softfail > result.txt
	  cleanup_self
          exit 1
       else
          notify_send "Build $PACKAGE_NAME on $BUILD_LOCATION failed."
...
$TOOLCHAIN_CMD ; check_fail
tar xf toolchain.tar.xz ; check_fail ; rm -f toolchain.tar.xz
$DEVICE_TREE_CMD ; check_fail
$VENDOR_TREE_CMD; check_fail
...
if [ "$BUILD_TYPE" == "vanilla" ]; then
   ...
   export BUILD_USERNAME=user        ; export BUILD_HOSTNAME=localhost
   export KBUILD_BUILD_USER=user     ; export KBUILD_BUILD_HOST=localhost
   ...
   notify_send "Build $PACKAGE_NAME on $BUILD_LOCATION succeeded."
   ...
   notify_send "Build $PACKAGE_NAME on $BUILD_LOCATION completed. $TIME_TAKEN."
...
#  notify_send "Build $PACKAGE_NAME GAPPS on $BUILD_LOCATION started."
...
# Notify
   #TIME_TAKEN=`printf '%dh:%dm:%ds\n' $((SECONDS/3600)) $((SECONDS%3600/60)) $((SECONDS%60))`
   #notify_send "Build $PACKAGE_NAME GAPPS on $BUILD_LOCATION completed. $TIME_TAKEN."
