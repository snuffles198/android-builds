...
# Setup lineage-24-BETA source
export GIT_CONFIG_PARAMETERS="'pull.rebase=true'"

rm -rf .repo/manifests
curl -o manifest-l24.tar.xz -L https://github.com/snuffles198/android-builds/raw/refs/heads/main/remote/src/manifest-l24.tar.xz ; check_fail
tar xvf manifest-l24.tar.xz ; check_fail

repo forall -c "git rebase --abort ; git cherry-pick --abort  ; git reset --hard HEAD ; git clean -fdx; git branch -D auto"
repo sync --nmu -c --force-sync --no-clone-bundle --optimized-fetch --no-tags --prune -d --force-checkout --force-remove-dirty

cd vendor/qcom/opensource/vibrator/
git switch lineage-24.0
cd -

./lineage/scripts/repopick/repopick.py -r -p -i -t "CP2A" 2>&1 | tee CP2A.log
./lineage/scripts/repopick/repopick.py -j 8 -r -p -i -Q "branch:lineage-24.0" 2>&1 | tee lineage-24.0.log

cd device/lineage/sepolicy/
wget https://github.com/LineageOS/android_device_lineage_sepolicy/commit/29ebee04ead17fb9c647e4e1219c83d8760574e7.patch
patch -p 1 -f < 29ebee04ead17fb9c647e4e1219c83d8760574e7.patch
cd -

for i in hardware/qcom-caf/sm8450-6.6/ hardware/qcom-caf/sm8750/ hardware/qcom-caf/sm8550 hardware/qcom-caf/sm8450 hardware/qcom-caf/sm8650 ; do
  for x in `find $i | grep Android.bp` ; do
    rm -fv $x
  done
done
...
echo 'diff --git a/qcwcn/wifi_hal/nan_pairing.cpp b/qcwcn/wifi_hal/nan_pairing.cpp
index c3f379a..ed6cfc1 100644
--- a/qcwcn/wifi_hal/nan_pairing.cpp
+++ b/qcwcn/wifi_hal/nan_pairing.cpp
@@ -2649,9 +2649,14 @@ void nan_pairing_set_password(struct nan_pairing_peer_info *peer, u8 *passphrase
     }
...
+    size_t pp_id_len = 0;
+    if (peer->sae_password_id)
+        pp_id_len = strlen(peer->sae_password_id);
+    pt = sae_derive_pt(NULL,
+                       pairing_ssid, pairing_ssid_len,
+                      (const u8 *)passphrase, len,
+                      (const u8 *)peer->sae_password_id,
+                      pp_id_len);
     pasn_set_pt(peer->pasn, pt);
     /* Set passpharse for Pairing Responder to validate PASN auth1 frame*/
     pasn_set_password(peer->pasn, peer->passphrase);' > 1.patch
patch -p 1 -f < 1.patch
cd -
